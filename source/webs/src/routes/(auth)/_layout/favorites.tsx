...
import { getAllFavoriteDocs, postDoc } from '@/services/document';
import { cn } from '@/lib/utils';


function RouteComponent() {
    const navigate = useNavigate({ from: '/favorites' });
    const { page, search, sortBy, sortOrder, limit } = useSearch({ from: '/(auth)/_layout/favorites' });
    ...
    const { isPending, error, data, isSuccess } = useQuery({
        queryKey: ['all-favorite-documents', page, search, sortBy, sortOrder, limit],
        queryFn: () => getAllFavoriteDocs({ page, search, sortBy, sortOrder, limit }),
    });
            ...
            data?.data?.length! < 1 ? (
                <Empty className="my-16">
                    <EmptyHeader>
                        ...
                        <EmptyTitle>No Documents in favorite Yet</EmptyTitle>
                        <EmptyDescription>
                            You haven&apos;t added any documents in favorite yet. Get started by adding
                            your first document in favorites.
                        </EmptyDescription>
                    </EmptyHeader>
                    <EmptyContent>
                        <Button disabled={createDocument.isPending} onClick={() => createDocument.mutate()}>Create Document</Button>
                    </EmptyContent>
                </Empty>
            ) : (
